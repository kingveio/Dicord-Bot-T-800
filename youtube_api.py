...
class YouTubeAPI:
    ...
    async def get_channel_id_from_url(self, url: str) -> Optional[str]:
        ...
        if len(path_parts) >= 2 and path_parts[0] == 'channel':
            ...
            return await self._get_id_from_name(channel_name, 'forUsername')
        elif len(path_parts) >= 1 and path_parts[0].startswith('@'):
            # https://www.youtube.com/@ChannelHandle
            channel_handle = path_parts[0][1:]
            return await self._get_id_from_name(channel_handle, 'forHandle')
    ...
    async def _get_id_from_name(self, name: str, search_type: str) -> Optional[str]:
        endpoint = f"{self.base_url}channels?part=id&{search_type}={name}&key={self.api_key}"
        try:
