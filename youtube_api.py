...
class YouTubeAPI:
    ...
    async def get_channel_live_status(self, channel_names: List[str]) -> Dict[str, bool]:
        ...
        A API do YouTube precisa do channelId, não do nome.
        Esta função primeiro resolve o nome para o ID e depois verifica a live.
        """
        live_status = {name.lower(): False for name in channel_names}
        
        # Primeiro, obter os channel IDs a partir dos nomes
        channel_ids = await self._get_channel_ids_by_name(channel_names)
        if not channel_ids:
            return live_status
            
        # Segundo, verificar o status de live usando os IDs
        ...
        try:
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    # Mapear os IDs dos canais de volta para seus nomes
