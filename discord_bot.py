...
class T800Bot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        logger.info(f"🤖 T-800 ONLINE. Missão iniciada em {self.start_time.strftime('%d/%m/%Y às %H:%M')}")
    ...
    async def check_live_status(self):
        ...
        if monitored_users["twitch"]:
            ...
            twitch_status = await self.twitch_api.check_live_status(twitch_logins)
    ...
    async def update_live_roles(self, guild: discord.Guild, twitch_lives: List[str], youtube_lives: List[str]):
        """Adiciona ou remove o cargo 'AO VIVO' para os membros."""
        live_role_obj = discord.utils.get(guild.roles, name=self.live_role)
        if not live_role_obj:
            try:
                live_role_obj = await guild.create_role(name=self.live_role, reason="Cargo para streamers ao vivo")
                logger.info(f"🆕 Cargo '{self.live_role}' criado no servidor '{guild.name}'")
            except discord.Forbidden:
                logger.error(f"❌ Não foi possível criar o cargo '{self.live_role}' no servidor '{guild.name}'. Verifique as permissões.")
                return
        ...
        for channel, info in all_monitored.items():
            ...
            if is_live and not has_role:
                try:
                    ...
                    logger.info(f"➕ Cargo '{self.live_role}' adicionado a {member.display_name}")
                except discord.Forbidden:
                ...
                try:
                    ...
                    logger.info(f"➖ Cargo '{self.live_role}' removido de {member.display_name}")
                except discord.Forbidden:
                    logger.error(f"❌ Não foi possível remover o cargo de {member.display_name}. Verifique as permissões.")

    # ========== COMANDOS ========== #
    @app_commands.command(name="status", description="Exibe o status atual do sistema.")
    async def status(self, interaction: discord.Interaction):
        ...
        status_msg = f"**Status do T-800**\n" \
                     f"Online: {(datetime.now() - self.start_time).total_seconds():.0f}s\n" \
                     f"Monitorando alvos: {monitored_count}"
        await interaction.followup.send(status_msg)

    @app_commands.command(name="adicionar", description="Adiciona um canal para monitoramento (Twitch/YouTube).")
    @app_commands.describe(url="URL do canal")
    async def add_target(self, interaction: discord.Interaction, url: str):
        await interaction.response.defer(ephemeral=True)
        platform = "youtube" if "youtube.com" in url or "youtu.be" in url else "twitch"

        if platform == "youtube":
            channel_id = await self.youtube_api.get_channel_id_from_url(url)
            if not channel_id:
            ...
            if not await self.twitch_api.validate_username(username):
    ...
    @app_commands.command(name="listar", description="Lista os canais e streamers monitorados.")
    ...
    async def test_live(self, interaction: discord.Interaction, channel_id: str):
        await interaction.response.defer(ephemeral=True)
        is_live = await self.youtube_api.is_channel_live(channel_id)
        if is_live:
